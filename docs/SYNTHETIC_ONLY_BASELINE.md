...
| # | Gap | Where |
|---|---|---|
| 1 | Audit writes are fire-and-forget inside `res.on('finish')`. A PHI export can succeed with neither the central nor the vault-side audit row written. No audit failure blocks any operation. | `server/audit-logger.js:420-423`, `server/routes/medivault-routes.js` |
| 2 | A PHI-bearing route added without a `PHI_ROUTES` classification is logged as `resource_type='unknown'` with no patient ID. Nothing fails the build. The table drifts silently. | `server/audit-logger.js` |
| 3 | SMART patient scopes are not bound to a patient compartment. A token with `patient/*.read` is not restricted to its launch patient. SMART must stay disabled. | `server/fhir/smart/scope-check.js` |
| 4 | The SMART token endpoint supports the password grant, exposes unauthenticated introspection, and advertises an empty JWKS. Prototype-only semantics. | `server/fhir/smart/token.js` |
| 5 | Clinical rules (CDS, HRT, peptide, functional medicine, triage, care-gap, dosing, contraindication) have no recorded source provenance — issuing authority, version, publication date, target population, exclusions, evidence strength, last-reviewed date. They are unvalidated and have no physician sign-off. | `server/agents/`, `server/pharma/`, `server/quality/` |
| 6 | Compounded peptides and BPC-157 content is present without an educational/draft-only quarantine flag. | `server/agents/domain-logic-agent.js` |
