...
| No outbound RxNorm lookups | `server/pharma/rxnorm-service.js` refuses every request unless `RXNORM_LIVE_ENABLED=true`. Only the `/interaction` endpoint had been gated; name-lookup, approximate-term and dosage-form calls reached `rxnav.nlm.nih.gov` unconditionally. Callers already tolerate a null result and fall back to the local curated data. | `RxNorm makes no outbound request unless explicitly enabled` |
...
| # | Gap | Where |
|---|---|---|
| 1 | **Largely closed.** Actions in `DURABLE_AUDIT_ACTIONS` (`EXPORT`, `SIGN`, `PRESCRIBE`) write their audit row **before** the handler runs, in every environment. If that write fails the operation is refused with 503 and nothing is disclosed. The caller receives an `X-Audit-Receipt` correlation id, and a row whose outcome is never recorded stays queryable via `findOrphanedAuditIntents` — a detectable gap rather than an absence. Routine `READ` deliberately keeps the fire-and-forget path: making every read block on a synchronous audit write would trade a silent gap for an availability failure across the whole API. Residual: reads can still lose an audit row silently, and the durable set is keyed on action, so a high-consequence route classified with the wrong action inherits the weaker path. | `server/audit-logger.js`, `test/unit/audit-durability.test.js` |
